...
namespace Assets
{
    class NeuralNetworkBackProp
    {
        ...
        public double[] GetOutput(double[] input)
        {
            ...
            double[] tanOut = new double[outputNeuronsCount];
            for (int i = 0; i < outputNeuronsCount; ++i)
            {
                tanOut[i] = HyperTan(outputSums[i]);
            }
            Array.Copy(tanOut, outputs, tanOut.Length);
        ...
        public void BackProp(double[][] trainData, int maxEpochs,
          double learnRate, double momentum, bool shuffle)
        {
            ...
            while (epoch < maxEpochs)
            {
                ...
                if (epoch % errInterval == 0 && epoch < maxEpochs)
                {
                    ...
                    UnityEngine.Debug.Log("epoch = " + epoch + "  error = " +
                ...
                for (int ii = 0; ii < trainData.Length; ++ii) //Dla każdego wektora uczącego
                {
                    ...
                    for (int k = 0; k < outputNeuronsCount; ++k)
                    {
                        ...
                        derivative = (1 + outputs[k]) * (1 - outputs[k]);
            ...
            System.IO.StreamWriter file = new System.IO.StreamWriter("error.txt");
        ...
        private double Error(double[][] trainData)
        {
            ...
            double[] xValues = new double[inputNeuronsCount]; // first numInput values in trainData
            double[] tValues = new double[outputNeuronsCount]; // last numOutput values

            // walk thru each training case. looks like (6.9 3.2 5.7 2.3) (0 0 1)
            for (int i = 0; i < trainData.Length; ++i)
            {
                Array.Copy(trainData[i], xValues, inputNeuronsCount);
                Array.Copy(trainData[i], inputNeuronsCount, tValues, 0, outputNeuronsCount); // get target values
                double[] yValues = this.GetOutput(xValues); // outputs using current weights
                for (int j = 0; j < outputNeuronsCount; ++j)
                {
